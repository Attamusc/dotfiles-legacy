...
class AutocompleteView extends SimpleSelectListView
  ...
  initialize: (@editorView) ->
    ...
    @addClass "autocomplete-plus-async"
    @providers = []
    ...
    @subscribeToCommand @editorView, "autocomplete-plus-async:activate", @runAutocompletion

    @on "autocomplete-plus-async:select-next", => @selectNextItemView()
    @on "autocomplete-plus-async:select-previous", => @selectPreviousItemView()
    @on "autocomplete-plus-async:cancel", => @cancel()
  ...
  currentFileBlacklisted: ->
    blacklist = (atom.config.get("autocomplete-plus-async.fileBlacklist") or "")
      .split ","
      .map (s) -> s.trim()
  ...
  runAutocompletion: =>
    ...
    suggestions = []
    cbCount = @providers.length
    for provider in @providers.slice().reverse()
      provider.buildSuggestions (list) =>
        ...
        if cbCount <= 0
          if suggestions && suggestions.length > 0
            ...
            @cancel() unless suggestions.length
  ...
  onChanged: (e) =>
    ...
    if typedText.length is 1 and atom.config.get "autocomplete-plus-async.enableAutoActivation"
      @contentsModified()
    else
